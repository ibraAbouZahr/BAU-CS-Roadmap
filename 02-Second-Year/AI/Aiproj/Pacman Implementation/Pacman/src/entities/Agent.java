...
public class Agent {
    ...
    public int depth; // for IDS

    public Agent() { this(null, 0., null, null , 0); }

    public Agent(State state) {
        this(state, 0., null, null, 0);
    }

    public Agent(State state, double cost, Agent parent, Action action , int depth) {
        this.state = state;
        this.cost = cost;
        this.parent = parent;
        this.action = action;
    ...
    public Agent transition(World world, Action action) {
        ...
        switch (action) {
            case Up: {
                return new Agent(new State(this.state.r - 1, this.state.c), this.cost + 1, this, action, 0);
            }
            case Down: {
                return new Agent(new State(this.state.r + 1, this.state.c), this.cost + 1, this, action,0);
            }
            case Left: {
                return new Agent(new State(this.state.r, this.state.c - 1), this.cost + 1, this, action,0);
            }
            case Right: {
                return new Agent(new State(this.state.r, this.state.c + 1), this.cost + 1, this, action,0);
            }
            default: {
                return new Agent(new State(this.state.r, this.state.c), this.cost, this, action,0);
            }
        }
    }
